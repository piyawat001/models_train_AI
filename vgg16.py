"""
VGG16 โมเดลสำหรับการจำแนกประเภทภาพรังสี
"""
...
# กำหนดการแปลงข้อมูลสำหรับการเทรนและการทดสอบ
...
class EarlyStopping:
    def __init__(self, patience=5, delta=0, verbose=False, path='VGG16_Best-Model.pt'):
...
class EarlyStoppingFineTune:
    def __init__(self, patience=5, delta=0, verbose=False, path='VGG16_Fine-Tune.pt'):
        self.patience = patience
        self.delta = delta
        self.verbose = verbose
        self.path = path
...
def test_model(model, dl_test, class_names, device, model_name="VGG16"):
    model.eval()
    y_true = []
    y_pred = []
...
# ฟังก์ชันหลัก
def main():
    print("โปรแกรมเทรนโมเดล VGG16 สำหรับจำแนกประเภทภาพรังสี")
    ...
    # ปรับชั้นสุดท้ายสำหรับการจำแนก 4 คลาส
    ...
    trained_model = train(
        ...
        epochs=NUM_EPOCHS,
        patience=PATIENCE,
        save_path=os.path.join(output_dir, 'VGG16_Best-Model.pt'),
        device=device
    )
    
    # ทดสอบโมเดลที่เทรนแล้ว
    print("\nทดสอบโมเดลที่เทรนแล้ว...")
    model.load_state_dict(torch.load(os.path.join(output_dir, 'VGG16_Best-Model.pt')))
    test_model(model, dl_test, train_dataset.class_names, device, "VGG16_Base")
    ...
    generate_prediction_excel(
        ...
        device=device,
        save_path=os.path.join(output_dir, 'VGG16_Base_Predictions.xlsx')
    )
    ...
    fine_tuned_model = train_fine_tune(
        ...
        epochs=NUM_EPOCHS,
        patience=PATIENCE,
        save_path=os.path.join(output_dir, 'VGG16_Fine-Tune.pt'),
        device=device,
    ...
    print("\nทดสอบโมเดลที่ fine-tune แล้ว...")
    model.load_state_dict(torch.load(os.path.join(output_dir, 'VGG16_Fine-Tune.pt')))
    test_model(model, dl_test, train_dataset.class_names, device, "VGG16_FineTuned")
    ...
    generate_prediction_excel(
        ...
        class_names=train_dataset.class_names,
        device=device,
        save_path=os.path.join(output_dir, 'VGG16_FineTuned_Predictions.xlsx')
    )
