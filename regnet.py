"""
RegNet โมเดลสำหรับการจำแนกประเภทภาพรังสี
"""
...
BATCH_SIZE = int(os.getenv('BATCH_SIZE', '16'))  # ค่าเริ่มต้นให้เหมาะสมกับ GPU 3080 ที่มี VRAM 10GB (RegNet ใช้หน่วยความจำมากกว่า)
...
FINE_TUNE_LEARNING_RATE = float(os.getenv('FINE_TUNE_LEARNING_RATE', '0.0001'))
...
# กำหนดการแปลงข้อมูลสำหรับการเทรนและการทดสอบ (RegNet ต้องการขนาดภาพ 384x384)
...
def show_images(images, labels, preds, class_names):
    ...
    plt.tight_layout()
    plt.savefig('prediction_examples_regnet.png')
    plt.close()

# คลาส EarlyStopping สำหรับบันทึกโมเดลที่ดีที่สุด
class EarlyStopping:
    def __init__(self, patience=5, delta=0, verbose=False, path='RegNet_Best-Model.pt'):
...
class EarlyStoppingFineTune:
    def __init__(self, patience=5, delta=0, verbose=False, path='RegNet_Fine-Tune.pt'):
        self.patience = patience
        self.delta = delta
        self.verbose = verbose
        self.path = path
...
def train(model, optimizer, loss_fn, dl_train, dl_test, test_dataset, epochs, patience, save_path, device):
    ...
    plt.tight_layout()
    plt.savefig('training_history_regnet.png')
    plt.close()
...
def train_fine_tune(model, optimizer, loss_fn, dl_train, dl_test, test_dataset, epochs, patience, save_path, device, threshold_percent=50):
    ...
    plt.tight_layout()
    plt.savefig('fine_tuning_history_regnet.png')
    plt.close()
    
    return model

# ฟังก์ชันสำหรับทดสอบโมเดล
def test_model(model, dl_test, class_names, device, model_name="RegNet"):
    model.eval()
    y_true = []
    y_pred = []
...
# ฟังก์ชันหลัก
def main():
    print("โปรแกรมเทรนโมเดล RegNet สำหรับจำแนกประเภทภาพรังสี")
    
    # สร้างโฟลเดอร์สำหรับผลลัพธ์
    output_dir = 'regnet_outputs'
    ...
    trained_model = train(
        ...
        epochs=NUM_EPOCHS,
        patience=PATIENCE,
        save_path=os.path.join(output_dir, 'RegNet_Best-Model.pt'),
        device=device
    )
    
    # ทดสอบโมเดลที่เทรนแล้ว
    print("\nทดสอบโมเดลที่เทรนแล้ว...")
    model.load_state_dict(torch.load(os.path.join(output_dir, 'RegNet_Best-Model.pt')))
    test_model(model, dl_test, train_dataset.class_names, device, "RegNet_Base")
    ...
    generate_prediction_excel(
        ...
        device=device,
        save_path=os.path.join(output_dir, 'RegNet_Base_Predictions.xlsx')
    )
    ...
    fine_tuned_model = train_fine_tune(
        ...
        epochs=NUM_EPOCHS,
        patience=PATIENCE,
        save_path=os.path.join(output_dir, 'RegNet_Fine-Tune.pt'),
        device=device,
    ...
    print("\nทดสอบโมเดลที่ fine-tune แล้ว...")
    model.load_state_dict(torch.load(os.path.join(output_dir, 'RegNet_Fine-Tune.pt')))
    test_model(model, dl_test, train_dataset.class_names, device, "RegNet_FineTuned")
    ...
    generate_prediction_excel(
        ...
        class_names=train_dataset.class_names,
        device=device,
        save_path=os.path.join(output_dir, 'RegNet_FineTuned_Predictions.xlsx')
    )
